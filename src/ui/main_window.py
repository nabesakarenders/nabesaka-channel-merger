...
from src.core.processor import merge_channels, save_image, normalize_to_8bit
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.success_sound.setSource(QUrl.fromLocalFile(assets_path / "sounds" / "success.wav"))
        self.success_sound.setVolume(0.5)
        
        self.error_sound = QSoundEffect()
        self.error_sound.setSource(QUrl.fromLocalFile(assets_path / "sounds" / "error.wav"))
        ...
        self.btn_export.setStyleSheet("background-color: #2c5d3f; font-weight: bold; font-size: 16px;")
        ...
        for w in [self.widget_r, self.widget_g, self.widget_b, self.widget_a]:
            ...
            if w.is_image_mode and w.current_image_data is not None:
                ...
                if res_key not in shapes:
                    shapes[res_key] = []
                shapes[res_key].append(w.title()) # Use widget title (e.g. "Red Output")
    ...
    def update_preview(self):
        ...
        if not is_valid:
            ...
            self.status_bar.setStyleSheet("QStatusBar { color: #ff5555; font-weight: bold; }")
            
            # Show concise error in Preview
            self.lbl_preview.setText("Resolution Error!\nSee details below.")
            self.lbl_preview.setStyleSheet("QLabel { color: #ff5555; font-weight: bold; background-color: #1a1a1a; border: 1px solid #553333; padding: 10px; }")
            return

        # Restore style / Clear error
        self.status_bar.showMessage("Ready")
        self.status_bar.setStyleSheet("") # Reset
        self.lbl_preview.setStyleSheet("background-color: #1a1a1a; border: 1px solid #333; color: #888;")
        ...
        try:
            ...
            scaled_pix = pixmap.scaled(w, h, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def export_image(self):
        ...
        # 16-bit check for user confirmation? Or just save.
        
        path, _ = QFileDialog.getSaveFileName(self, "Save Texture", self.last_directory, "PNG Image (*.png);;Targa (*.tga);;TIFF (*.tiff);;EXR (*.exr)")
        if path:
            self.last_directory = str(Path(path).parent)
            success = save_image(path, merged)
            if success:
                ...
                self.status_bar.setStyleSheet("QStatusBar { color: #55ff55; font-weight: bold; }")
