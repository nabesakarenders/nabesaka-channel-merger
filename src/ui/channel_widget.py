from PySide6.QtWidgets import (
    ...
    QFileDialog, QButtonGroup, QToolButton, QGroupBox,
    QFrame
)
...
class ChannelWidget(QGroupBox):
    ...
    def init_ui(self):
        ...
        self.combo_channel.currentIndexChanged.connect(lambda: self.contentChanged.emit())
        ...
        self.lbl_thumbnail.setStyleSheet("background-color: #222; border: 1px solid #444; font-size: 16px; color: #777;")
        ...
        self.value_preview.setStyleSheet(f"background-color: rgb({default_val}, {default_val}, {default_val});")
    ...
    def update_value_preview(self, value):
        self.value_preview.setStyleSheet(f"background-color: rgb({value}, {value}, {value});")
    ...
    def load_file_dialog(self):
        path, _ = QFileDialog.getOpenFileName(self, "Open Image", self.window().last_directory, "Images (*.png *.jpg *.jpeg *.tga *.bmp *.exr *.tif *.tiff)")
        if path:
    ...
    def populate_channel_combo(self, img_data):
        ...
        target_idx = -1
        if self.color_code == "R": target_idx = 0
        elif self.color_code == "G": target_idx = 1
        elif self.color_code == "B": target_idx = 2
        elif self.color_code == "A": target_idx = 3
